(function () {
    // load prefix tree to store jsLib for instant search results
  var prefix = new trie();

  $(document).ready(function () {
    LoadScripts();

    // by default have input box be selected
    $('.input').focus();


    // Set it so input box is always in focus
    $('.input').blur(function () {
      $('.input').focus();
    });


    // Whenever input is updated, update the search results
    $('.input').on('input', function () {
      DislayLeads($(this).val());
  ...
  // Load all the scripts and store them in pefix tree
  ...
  // Create DOM ELEMENT for each script to dislay
